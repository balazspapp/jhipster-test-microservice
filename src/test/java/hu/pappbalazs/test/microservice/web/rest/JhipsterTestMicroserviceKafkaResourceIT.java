...
import hu.pappbalazs.test.microservice.IntegrationTest;
import hu.pappbalazs.test.microservice.config.EmbeddedKafka;
import hu.pappbalazs.test.microservice.config.KafkaSseConsumer;
import hu.pappbalazs.test.microservice.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class JhipsterTestMicroserviceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jhipster-test-microservice-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhipster-test-microservice-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/jhipster-test-microservice-kafka/unregister"));
                return;
            }
        }
